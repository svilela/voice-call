...
import pandas as pd
from behave import given, model, step, then, when  # type: ignore
...
@given("que temos o behave instalado")
...
def a_silly_step(context):
    logger.info("STEP: Given a simple silly step")
...
def final_table(context):
    ...
    for idx, row in enumerate(table.rows):
        ...
        logger.info(
            "STEP: Then the last step has a final table -> "
...
def read_machine_13(context):
    logger.info("STEP: Given a dataset in the Parquet file representing a time series")
    f = "data/parquet/machine13_01.parquet"
...
def step_impl(context):
    logger.info("STEP: Then I plot 4 features selected for exploratory analysis")
    do_plot(context.df)
